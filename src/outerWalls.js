 ...
 */
export function makeWalls(width, length, wallHeight, doorLocs, doorSize, baseName, wOffset = 0, lOffset = 0, hOffset = 0, vOffset = 0) {
    ...
    for (let i = 0; i < doorLocs.length; i++) {
        ...
        doorLocs[i].forEach(wallSeg => { //LTRB
            switch (i) {
                case 0:
                    vOffset = wallGenerator(wallSeg, wallWidth, wallHeight, baseName, {}, -width/2 + wallSeg/2 + lengthCovered + wOffset, -length/2 + lOffset, wallHeight/2 + hOffset, vOffset);
                    break;
                case 1:
                    vOffset = wallGenerator(wallWidth, wallSeg, wallHeight, baseName, {}, width/2 + wOffset, -length/2 + wallSeg/2 + lengthCovered + lOffset, wallHeight/2 + hOffset, vOffset);
                    break;
                case 2:
                    vOffset = wallGenerator(wallSeg, wallWidth, wallHeight, baseName, {}, -width/2 + wallSeg/2 + lengthCovered + wOffset, length/2 + lOffset, wallHeight/2 + hOffset, vOffset);
                    break;
